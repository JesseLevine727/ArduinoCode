...
void BLEscan(){
  ...
  Serial.println("BLE Central - Music Converter Proto 3");
...
void ESP32Chars(BLEDevice peripheral) {
  ...
  BLECharacteristic Drum = peripheral.characteristic("e384b790-ff7b-11eb-9a03-0242ac130003");
  BLECharacteristic Drum2 = peripheral.characteristic("86e327fa-0044-11ec-9a03-0242ac130003");
  BLECharacteristic Drum3 = peripheral.characteristic("02ec68de-0045-11ec-9a03-0242ac130003");
  ...
  while (peripheral.connected()) {
   ...
   if(Drum.valueUpdated()) {
      char DrumBuff[1];

      Drum.readValue(DrumBuff,1);
      
      //convert TouchBuff to a string which then gets converted to a floating point number
      int Drumsize = sizeof(DrumBuff) / sizeof(char);
      String DrumString = convertToString(DrumBuff,Drumsize);
      //Serial.print(convertToFloat(DrumString));
      
      Serial.print('\n');
      
      //When the touch pad had been touched, the program will stop running
    if(convertToInt(DrumString) == 1){
       Keyboard.press('0');
       delay(10);
       Keyboard.releaseAll();
    ...
    if(Drum2.valueUpdated()) {
      char Drum2Buff[1];

      Drum2.readValue(Drum2Buff,1);
      
      //convert TouchBuff to a string which then gets converted to a floating point number
      int Drum2size = sizeof(Drum2Buff) / sizeof(char);
      String Drum2String = convertToString(Drum2Buff,Drum2size);
            
      Serial.print('\n');
      
      //When the touch pad had been touched, the program will stop running
    if(convertToInt(Drum2String) == 1){
       Keyboard.press('7');
       delay(10);
       Keyboard.releaseAll();
    ...
    if(Drum3.valueUpdated()) {
      char Drum3Buff[1];

      Drum3.readValue(Drum3Buff,1);
      
      //convert TouchBuff to a string which then gets converted to a floating point number
      int Drum3size = sizeof(Drum3Buff) / sizeof(char);
      String Drum3String = convertToString(Drum3Buff,Drum3size);
            
      Serial.print('\n');
      
      //When the touch pad had been touched, the program will stop running
    if(convertToInt(Drum3String) == 1){
       Keyboard.press('4');
       delay(10);
       Keyboard.releaseAll();
